...
/* ============================================================
func name   :   neul_bc95_set_cdp_info
...
Revision    : 
=============================================================== */
static int neul_bc95_set_cdp_info(const char *cdp)
{
    if(NULL == cdp)
    {
        return -1;
...
int neul_bc95_hw_detect(void)
{
    ...
    str = strstr(neul_dev.rbuf,"Neul Hi2110");
    if (!str)
    {
...
Revision    : 
=============================================================== */
static int neul_bc95_hex_to_str(const unsigned char *bufin, int len, char *bufout)
{
    int i = 0;
    #if 0
    int tmp = 0;
...
int neul_bc95_send_coap_paylaod(const char *buf, int sendlen)
{
    ...
    ret = neul_bc95_hex_to_str((unsigned char *)buf, sendlen, neul_bc95_tmpbuf);
...
int neul_bc95_read_coap_msg(char *outbuf, int maxrlen)
{
    ...
    if (readlen > 0)
    {
        neul_bc95_str_to_hex(neul_bc95_tmpbuf, readlen*2, outbuf);
        return readlen;
    }
    return 0;
}
...
int neul_bc95_udp_read(int socket,char *buf, int maxrlen, int mode)
{
    ...
    sscanf(neul_dev.rbuf, "%d,%s,%d,%d,%s,%d\r%s", &rskt,
                            neul_bc95_tmpbuf,
                            &port,
                            &rlen,
                            neul_bc95_tmpbuf+15,
...
int uart_data_read(char *buf, int maxrlen, int mode, int timeout)
{
    ...
    {
        //block mode
        retlen = los_dev_uart_read(LOS_STM32L476_UART2, buf, maxrlen, timeout);
    }
    else
    {
        //none block mode
        retlen = los_dev_uart_read(LOS_STM32L476_UART2, buf, maxrlen, 0);
    }
...
int uart_data_write(char *buf, int writelen, int mode)
{
    ...
    los_dev_uart_write(LOS_STM32L476_UART2, buf, writelen, 500);
    return 0;
}
